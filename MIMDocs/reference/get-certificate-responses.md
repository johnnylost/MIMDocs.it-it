---
title: Ottenere le risposte di certificato | Microsoft Docs
description: 
keywords: 
author: msmbaldwin
ms.author: mbaldwin
manager: mbaldwin
ms.date: 10/17/2016
ms.topic: reference
ms.prod: identity-manager-2015
ms.service: microsoft-identity-manager
ms.technology: security
ms.assetid: b0559e7b-eaff-499b-8bcf-c6cdf5a89545
audience: developer
ms.reviewer: mwahl
ms.suite: ems
ms.openlocfilehash: 3ac905a25e41a6212efb5f59e2f47e4345382ec3
ms.sourcegitcommit: 02fb1274ae0dc11288f8bd9cd4799af144b8feae
ms.translationtype: HT
ms.contentlocale: it-IT
ms.lasthandoff: 07/13/2017
---
# <a name="get-certificate-responses"></a>Get Certificate Responses
Invia una serie di richieste di certificati all'autorità di certificazione (CA) per l'elaborazione.

**Nota**: il numero di risposte certificato può superare il numero di richieste in quanto alcuni modelli di profilo possono essere configurate per generare chiavi/richieste sul server oppure è possibile disporre di certificati esterni.

**Nota**: gli URL mostrati in questo argomento sono relativi al nome host scelto durante la distribuzione delle API. Ad esempio: `https://api.contoso.com`.
##<a name="request"></a>Richiesta


Metodo  |URL della richiesta  
---------|---------
POST     |/CertificateManagement/api/v1.0/requests/{ID}/certificatedata

###<a name="url-parameters"></a>Parametri URL
Parametro | Descrizione
--------|--------------
ID| Obbligatorio. Il GUID corrispondente al modello di profilo da cui deve essere estratto il criterio.

###<a name="request-headers"></a>Intestazioni delle richieste
Per le intestazioni di richiesta comuni, vedere [HTTP Request and Response Headers](certificate-management-rest-api-service-details.md#http-request-and-response-headers) (Intestazioni di richiesta e risposta HTTP) in *Dettagli del servizio API REST di gestione certificati*.
###<a name="request-body"></a>Testo della richiesta
Il testo della richiesta contiene le proprietà seguenti.

Proprietà | Descrizione
---------|-----------
pfxpassword | La password utilizzata per proteggere i risultati PFX (Personal Information) che vengono restituiti dalla chiamata.
certificaterequests | L’elenco di richieste di certificati generato dal client. Queste richieste devono essere state generate usando i parametri specificati nell'oggetto CertificateRequestGenerationOptions. Per altre informazioni, vedere [Ottenere le opzioni di generazione della richiesta di certificato](get-certificate-request-generation-options.md).
comments | Stringa contenente eventuali commenti aggiuntivi che saranno registrati nella cronologia eventi dell'operazione.


##<a name="response"></a>Risposta
###<a name="response-codes"></a>Codici di risposta
Codice  |Descrizione  
---------|---------
200     | OK
403 | Non consentito
500 | Errore interno

###<a name="response-headers"></a>Intestazioni delle risposte
Per le intestazioni di risposta comuni, vedere [HTTP Request and Response Headers](certificate-management-rest-api-service-details.md#http-request-and-response-headers) (Intestazioni di richiesta e risposta HTTP) in *Dettagli del servizio API REST di gestione certificati*.
###<a name="response-body"></a>Testo della risposta
In caso di esito positivo, restituisce un elenco di oggetti serializzati JSON [Microsoft.Clm.Shared.Certificates.Certificate](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx) con le proprietà seguenti:

Proprietà| Descrizione
------|-------------
CertificateId | L’ID del certificato.
Errore di | Qualsiasi errore verificatosi durante l'elaborazione della richiesta di certificato.
Flag | I flag che descrivono il certificato.
IsExternal | Valore booleano che indica se il certificato è un certificato esterno.
isKeyHistory | Valore booleano che indica se il certificato è certificato di cronologia delle chiavi.
isPfx | Valore booleano che indica se la risposta alla richiesta di certificato è in formato PFX.
isPkcs7 | Valore booleano che indica se la risposta alla richiesta di certificato è in formato PKCS#7.
isServerGenerated | Valore booleano che indica se il certificato è stato generato da un server.
Pfx | BLOB del certificato PFX.
Pkcs7 | BLOB del certificato PKCS#7.
TemplateCommonName | Il nome comune del modello del certificato per il certificato.
UseLocalMachineStore | Valore booleano che indica se il certificato è per l’archivio di certificati del computer.

##<a name="example"></a>Esempio

###<a name="request"></a>Richiesta
```
POST /CertificateManagement/api/v1.0/requests/C6BAD97C-F97F-4920-8947-BE980C98C6B5/certificatedata HTTP/1.1

{
    "pfxpassword":"6b75162c-b3b4-4fc1-a653-f12dae98f0c3",
    "certificaterequests":[
        "\r\n"
    ]
}

```
###<a name="response"></a>Risposta
```
HTTP/1.1 200 OK

[
    {
        "TemplateCommonName":"ContosoVirtualSmartCardLimitedRelease-KSP",
        "CertificateId":0,
        "Flags":65536,
        "UseLocalMachineStore":false,
        "Error":null,
        "Pkcs7":"\r\n",
        "Pfx":null,
        "IsExternal":false,
        "isServerGenerated":false,
        "isPfx":false,
        "isPkcs7":true,
        "isKeyHistory":false
    }
]
```       
##<a name="see-also"></a>Vedere anche

- [Metodo Microsoft.Clm.Provision.ExecuteOperations.Enroll](https://msdn.microsoft.com/library/microsoft.clm.provision.executeoperations.enroll.aspx)
- [Classe Microsoft.Clm.Shared.Certificates.Certificate](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx)
